        ...
        <div class="row pb-5 mt-5 mb-5 mx-1">
            ...
            <div class="editProfile-table mb-5 table-responsive">
                <table class="table your-clicks">
                    ...
                    <tbody>
                        @foreach ($clicked_by_me as $data)
                        <tr>
                            <td style="white-space: nowrap;">
                            @foreach($referral_links_all as $referral_links)
                                @if($referral_links->id==$data->referral_id)
                                {{$referral_links->display_name}}
                                @endif
                                @endforeach
                            {{ $data->display_name }}</td>
                            <td style="white-space: nowrap;">{{ $data->date_pst }}</td>
                            <td style="white-space: nowrap;">
                            @foreach($all_users as $users)
                                @if($users->id==$data->referrer_user_id)
                                    {{$users->username}}
                                @endif
                            @endforeach    
                            {{ $data->user_id }}</td>
                            ...
                            <td style="white-space: nowrap;">
                                @if($data->logo)
                                <a href="{{ asset('storage/' . $data->logo) }}" target="_blank">View Screenshot</a>
                                @else
